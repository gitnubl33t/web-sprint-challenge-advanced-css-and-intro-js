...
function getArtistByIndex(array, index) {
  let str = "";
  str = `The artist at index ${index} is ${array[index].name}`;
  return str;
}
...
function get20s(array) {
  ...
  for (i = 0; i < array.length; i++) {
    lifeYears = array[i].years;

    if (lifeYears.charAt(1) === "9" && lifeYears.charAt(8) === "9") {
      deaths20s.push(array[i].name);
    }
  }
  console.log(deaths20s);
}
